document.addEventListener('DOMContentLoaded', async function () {
	...
	async function getInvoices() {
		...
		res = res.map((v) => {
			return {
				id: v.id,
				invoiceDate: v.ngayLapHoaDon,
				room: v.soPhong,
				electricity: v.tienDien,
				water: v.tienNuoc,
				roomPrice: v.donGia,
				finalPayment: v.tongTien,
			};
		});
		return res;
	}
			...
			invoiceDiv.innerHTML = `
                <div class="room-product-main">
                    <div class="room-product-bottom">
                        <h4><a href="#">Ngày lập: ${invoice.invoiceDate.substring(0, 10)}</a></h4>
	...
	document.getElementById('addInvoiceForm').addEventListener('submit', async function (event) {
		...
		if (mode === 'add') {
			var newInvoice = {
				ngayLapHoaDon: new Date(invoiceDate),
				phong_id: room,
				soPhong: room,
			...
			if (index !== -1) {
				...
				let res = await fetch(`http://localhost:8000/api/hoaDon/${id}`, {
					method: 'PUT',
					headers: {
						'Content-Type': 'application/json',
					},
	...
	document.getElementById('invoiceList').addEventListener('click', async function (event) {
		if (event.target.classList.contains('edit-btn')) {
			...
			if (invoice) {
				document.getElementById('invoiceDate').value = invoice.invoiceDate.substring(0, 10);
		...
		if (event.target.classList.contains('delete-btn')) {
			...
			if (confirm('Bạn có chắc chắn muốn xóa hóa đơn thanh toán này?')) {
				await fetch(`http://localhost:8000/api/hoaDon/${id}`, {
					method: 'DELETE',
				});
                invoices = await getInvoices();
                loadInvoiceList();
